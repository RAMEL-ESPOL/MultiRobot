...
def generate_launch_description():
    ...
    gazebo = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(get_package_share_directory('mrs_master'), 'launch', 'gazebo_wh.launch.py'))
    )

    # Specify the path to the second launch file
    multi_robot = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(get_package_share_directory('mrs_master'), 'launch', 'multi_create_wh.launch.py'))
    ...
    multi_cam = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(get_package_share_directory('mrs_master'), 'launch', 'multi_cam_simulation_wh.launch.py')), launch_arguments={'number': LaunchConfiguration('number'),}.items()
    )
