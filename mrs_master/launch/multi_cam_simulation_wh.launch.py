...
def cameras_generator(context, *args, **kwargs):
    ...
    for i in range(int(number_of_cameras)):
        ...
        cam_name = "c"+str(i)
        y_pos = float(i)
        cameras.append({'name': cam_name, 'namespace': cam_name, 'x_pose': 0.0, 'y_pose': y_pos*2-8.0, 'z_pose': 3.50})
    print(cameras)
    
    
    cameras.append({'name': "c6", 'namespace': "c6", 'x_pose': -4.0, 'y_pose': 0.0, 'z_pose': 4.0})
